...
Example: passiveid-rest.py -n psn1.company.com -a admin -p passwd -u jsmith -i 1.2.3.4 -d  example.com -v"""
)
	...
	if r.status_code==201:
		...
		try:         
			r=requests.post(f"https://{args.n}:9094/api/fmi_platform/v1/identityauth/generatetoken",
				auth=requests.auth.HTTPBasicAuth(args.a,args.p),
				verify=False)
		except Exception as e:
			logger.error(f"Connection to PSN failed: {e}")
			break
		if r.status_code==204:
			...
			tokenfile.close()
			logger.info("Authentication Token retrieved. Saved to passiveid_token.txt")
		else:
